...
const students = sqliteTable("students", {
  ...
  nameIdx: index$1("students_name_idx").on(table.firstName, table.lastName),
  kickedIdx: index$1("students_kicked_idx").on(table.isKicked)
}));
const subjects = sqliteTable("subjects", {
  ...
  titleIdx: index$1("subjects_title_idx").on(table.title)
}));
const groups = sqliteTable("groups", {
  ...
  subjectIdIdx: index$1("groups_subject_id_idx").on(table.subjectId),
  nameIdx: index$1("groups_name_idx").on(table.name)
}));
const enrollments$1 = sqliteTable("enrollments", {
  ...
  studentIdIdx: index$1("enrollments_student_id_idx").on(table.studentId),
  groupIdIdx: index$1("enrollments_group_id_idx").on(table.groupId),
  studentGroupIdx: index$1("enrollments_student_group_idx").on(table.studentId, table.groupId),
  statusIdx: index$1("enrollments_status_idx").on(table.status)
}));
const subscriptions = sqliteTable("subscriptions", {
  ...
  studentIdIdx: index$1("subscriptions_student_id_idx").on(table.studentId),
  subjectIdIdx: index$1("subscriptions_subject_id_idx").on(table.subjectId),
  monthIdx: index$1("subscriptions_month_idx").on(table.month),
  statusIdx: index$1("subscriptions_status_idx").on(table.status)
}));
const payments = sqliteTable("payments", {
  ...
  studentIdIdx: index$1("payments_student_id_idx").on(table.studentId),
  subjectIdIdx: index$1("payments_subject_id_idx").on(table.subjectId),
  dateIdx: index$1("payments_date_idx").on(table.date)
}));
const settings = sqliteTable("settings", {
  ...
  keyIdx: index$1("settings_key_idx").on(table.key)
}));
...
const auditLogs = sqliteTable("audit_logs", {
  ...
  actionIdx: index$1("audit_logs_action_idx").on(table.action),
  entityTypeIdx: index$1("audit_logs_entity_type_idx").on(table.entityType),
  entityIdIdx: index$1("audit_logs_entity_id_idx").on(table.entityId)
}));
      ...
      return try_$2({
        try: async () => {
          ...
          try {
            ...
            logger$1.info({ count: (allSubjects == null ? void 0 : allSubjects.length) || 0, rawSubjects: allSubjects }, "Raw subjects from database");
            ...
            const subjectsWithStats = await Promise.all(allSubjects.map(async (subject) => {
              ...
              try {
                ...
                try {
                  ...
                  logger$1.warn({ subject: subject.id, err: err2 }, "Failed to get group count");
                }
                let studentCount = 0;
                try {
                  ...
                  logger$1.warn({ subject: subject.id, err: err2 }, "Failed to get student count");
                }
                return {
                  ...subject,
                  groupCount,
                ...
                logger$1.warn({ subject: subject.id, err: subjectError }, "Failed to get stats for subject");
                return {
                  ...subject,
                  groupCount: 0,
                  studentCount: 0
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("group:directCreateGroup", async (_, subjectId, data) => {
    try {
      ...
      const insertedGroup = await db2.select().from(groups2).where((eq2) => eq2(groups2.id, id)).then((rows) => rows[0]);
      if (!insertedGroup) {
  ...
  electron.ipcMain.handle("subject:directDeleteSubject", async (_, id) => {
    ...
    try {
      const { db: db2 } = await Promise.resolve().then(() => index);
      const { subjects: subjects2 } = await Promise.resolve().then(() => schema);
      const { eq: eq2 } = await Promise.resolve().then(() => require("./index-CVcoWc1B.js"));
