...
function fastRedact$1(opts = {}) {
  ...
  if (paths.length === 0) return serialize2 || noop$4;
...
function requireTransport() {
  ...
  const { join: join2, isAbsolute, sep } = path$2;
...
const logsPath$3 = path$2.join(userDataPath$3, "logs");
if (!fs$2.existsSync(logsPath$3)) {
  fs$2.mkdirSync(logsPath$3, { recursive: true });
}
...
}, pino$1.destination(path$2.join(logsPath$3, "ipc.log")));
...
class SQLiteDialect {
  ...
  buildUpdateQuery({ table: table2, set: set2, where, returning, withList }) {
    const withSql = this.buildWithCTE(withList);
    const setSql = this.buildUpdateSet(table2, set2);
    const returningSql = returning ? sql` returning ${this.buildSelection(returning, { isSingleTable: true })}` : void 0;
    const whereSql = where ? sql` where ${where}` : void 0;
...
class SQLiteUpdateBase extends (_Ib = QueryPromise, _Hb = entityKind, _Ib) {
  constructor(table2, set2, session, dialect, withList) {
    super();
    /** @internal */
    __publicField(this, "config");
    __publicField(this, "run", (placeholderValues) => {
    ...
    this.session = session;
    this.dialect = dialect;
    this.config = { set: set2, table: table2, withList };
  }
  /**
   * Adds a 'where' clause to the query.
   *
...
const students = sqliteTable("students", {
  ...
  nameIdx: index$1("students_name_idx").on(table2.firstName, table2.lastName),
  kickedIdx: index$1("students_kicked_idx").on(table2.isKicked)
}));
const subjects = sqliteTable("subjects", {
  ...
  titleIdx: index$1("subjects_title_idx").on(table2.title)
}));
const groups = sqliteTable("groups", {
  ...
  subjectIdIdx: index$1("groups_subject_id_idx").on(table2.subjectId),
  nameIdx: index$1("groups_name_idx").on(table2.name)
}));
const enrollments$1 = sqliteTable("enrollments", {
  ...
  studentIdIdx: index$1("enrollments_student_id_idx").on(table2.studentId),
  groupIdIdx: index$1("enrollments_group_id_idx").on(table2.groupId),
  studentGroupIdx: index$1("enrollments_student_group_idx").on(table2.studentId, table2.groupId),
  statusIdx: index$1("enrollments_status_idx").on(table2.status)
}));
const subscriptions = sqliteTable("subscriptions", {
  ...
  studentIdIdx: index$1("subscriptions_student_id_idx").on(table2.studentId),
  subjectIdIdx: index$1("subscriptions_subject_id_idx").on(table2.subjectId),
  monthIdx: index$1("subscriptions_month_idx").on(table2.month),
  statusIdx: index$1("subscriptions_status_idx").on(table2.status)
}));
const payments = sqliteTable("payments", {
  ...
  studentIdIdx: index$1("payments_student_id_idx").on(table2.studentId),
  subjectIdIdx: index$1("payments_subject_id_idx").on(table2.subjectId),
  dateIdx: index$1("payments_date_idx").on(table2.date)
}));
const settings = sqliteTable("settings", {
  ...
  keyIdx: index$1("settings_key_idx").on(table2.key)
}));
...
const auditLogs = sqliteTable("audit_logs", {
  ...
  actionIdx: index$1("audit_logs_action_idx").on(table2.action),
  entityTypeIdx: index$1("audit_logs_entity_type_idx").on(table2.entityType),
  entityIdIdx: index$1("audit_logs_entity_id_idx").on(table2.entityId)
}));
...
function getNativeModulePath(moduleName) {
  ...
  if (isDev2) {
    return path$2.join(process.cwd(), "node_modules", moduleName);
  }
  const appPath = electron.app.getAppPath();
  const isAsar = appPath.includes("app.asar");
  if (isAsar) {
    const unpackedPath = appPath.replace("app.asar", "app.asar.unpacked");
    return path$2.join(unpackedPath, "node_modules", moduleName);
  }
  return path$2.join(process.cwd(), "node_modules", moduleName);
}
function getAlternativePaths(moduleName) {
  return [
    ...
    path$2.join(electron.app.getAppPath(), "node_modules", moduleName),
    // Try node_modules in app parent directory
    path$2.join(electron.app.getAppPath(), "..", "node_modules", moduleName),
    // Try node_modules in current working directory
    path$2.join(process.cwd(), "node_modules", moduleName),
    // Try absolute path for development
    path$2.resolve("./node_modules", moduleName),
    // Try electron app resources directory
    path$2.join(process.resourcesPath || "", "node_modules", moduleName),
    // Try electron app resources/app directory
    path$2.join(process.resourcesPath || "", "app", "node_modules", moduleName),
    // Try electron app resources/app.asar.unpacked directory
    path$2.join(process.resourcesPath || "", "app.asar.unpacked", "node_modules", moduleName)
  ];
}
function moduleExists(modulePath) {
  try {
    if (fs$2.existsSync(modulePath)) {
      return true;
    }
    const packageJsonPath = path$2.join(modulePath, "package.json");
    if (fs$2.existsSync(packageJsonPath)) {
      const packageJson = JSON.parse(fs$2.readFileSync(packageJsonPath, "utf8"));
      const mainFile = packageJson.main || "index.js";
      const mainPath = path$2.join(modulePath, mainFile);
      return fs$2.existsSync(mainPath);
    }
    return false;
  } catch (error) {
...
const dbPath$1 = path$2.join(userDataPath$2, "scholario.db");
const logsPath$2 = path$2.join(userDataPath$2, "logs");
if (!fs$2.existsSync(logsPath$2)) {
  fs$2.mkdirSync(logsPath$2, { recursive: true });
}
const logger$7 = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsPath$2, "db.log")));
function initializeDatabase$1() {
  try {
    logger$7.info(`Initializing database at ${dbPath$1}`);
    if (!fs$2.existsSync(path$2.dirname(dbPath$1))) {
      fs$2.mkdirSync(path$2.dirname(dbPath$1), { recursive: true });
    }
    logger$7.info(`Current working directory: ${process.cwd()}`);
    logger$7.info(`User data path: ${userDataPath$2}`);
    logger$7.info(`Node.js version: ${process.version}`);
    logger$7.info(`Electron version: ${process.versions.electron}`);
    logger$7.info("Loading better-sqlite3 module");
    const Database2 = loadNativeModule("better-sqlite3");
    if (!Database2) {
      throw new Error("Failed to load better-sqlite3 module");
    }
    logger$7.info("Creating SQLite database instance");
    let sqlite;
    try {
      sqlite = new Database2(dbPath$1);
      logger$7.info("SQLite database instance created successfully");
    } catch (dbError) {
      logger$7.error({ err: dbError }, "Failed to create SQLite database instance");
      logger$7.info("Trying in-memory database as fallback");
      sqlite = new Database2(":memory:");
      logger$7.info("In-memory SQLite database instance created successfully");
    }
    try {
      sqlite.pragma("journal_mode = WAL");
      sqlite.pragma("foreign_keys = ON");
      logger$7.info("SQLite pragmas set successfully");
    } catch (pragmaError) {
      logger$7.error({ err: pragmaError }, "Failed to set SQLite pragmas");
    }
    const db2 = drizzle(sqlite, { schema });
    logger$7.info("Drizzle instance created successfully");
    try {
      const testQuery = sqlite.prepare("SELECT 1 AS test").get();
      logger$7.info({ testResult: testQuery }, "Database test query executed successfully");
    } catch (testError) {
      logger$7.error({ err: testError }, "Database test query failed");
    }
    logger$7.info("Database initialized successfully");
    return db2;
  } catch (error) {
    logger$7.error({ err: error }, "Failed to initialize database");
    throw error;
  }
}
...
try {
  ...
  try {
    ensureDatabaseSetup().then((success) => {
      if (success) {
        logger$7.info("Database tables created successfully");
      } else {
        logger$7.error("Failed to create database tables");
      }
    }).catch((error) => {
      logger$7.error({ err: error }, "Error during database setup");
    });
  } catch (error) {
    logger$7.error({ err: error }, "Critical error during database setup");
  }
} catch (err2) {
  logger$7.error({ err: err2 }, "Failed to load migration module");
}
...
const index = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  db,
  generateUlid: generateUlid$1,
  initializeDatabase: initializeDatabase$1
}, Symbol.toStringTag, { value: "Module" }));
const logsDir$4 = path$2.join(electron.app.getPath("userData"), "logs");
if (!fs$2.existsSync(logsDir$4)) {
  fs$2.mkdirSync(logsDir$4, { recursive: true });
}
const logger$6 = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsDir$4, "auth-service.log")));
...
const AuthService = {
  ...
  initialize: () => try_({
    try: async () => {
      logger$6.info("Initializing authentication service");
      const existingUser = await db.select().from(users).limit(1).then((rows) => rows[0]);
      if (!existingUser) {
        logger$6.info("No users found, creating default user");
        await db.insert(users).values({
          id: generateUlid$1(),
          accessCode: DEFAULT_ACCESS_CODE
        });
        logger$6.info("Default user created successfully");
      } else {
        logger$6.info("Existing user found, skipping default user creation");
      }
      return true;
    },
    catch: (error) => {
      logger$6.error({ err: error }, "Failed to initialize authentication service");
  ...
  validateAccessCode: (code) => try_({
    try: async () => {
      logger$6.info("Validating access code");
      const user = await db.select().from(users).where(eq(users.accessCode, code)).then((rows) => rows[0]);
      const isValid = !!user;
      logger$6.info({ isValid, code }, "Access code validation result");
      return isValid;
    },
    catch: (error) => {
      logger$6.error({ err: error }, "Failed to validate access code");
  ...
  updateAccessCode: (newCode) => try_({
    try: async () => {
      logger$6.info("Updating access code");
      const existingUser = await db.select().from(users).limit(1).then((rows) => rows[0]);
      if (!existingUser) {
        logger$6.warn("No user found when updating access code");
      ...
      logger$6.info("Access code updated successfully");
      return true;
    },
    catch: (error) => {
      logger$6.error({ err: error }, "Failed to update access code");
...
const dbPath = path$2.join(userDataPath$1, "scholario.db");
const logsPath$1 = path$2.join(userDataPath$1, "logs");
if (!fs$2.existsSync(logsPath$1)) {
  fs$2.mkdirSync(logsPath$1, { recursive: true });
}
const logger$5 = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsPath$1, "db.log")));
let dbInstance = null;
let connectionState = {
  connected: false,
  error: null,
...
function initializeDatabase() {
  try {
    logger$5.info(`Initializing database at ${dbPath}`);
    if (!fs$2.existsSync(path$2.dirname(dbPath))) {
      fs$2.mkdirSync(path$2.dirname(dbPath), { recursive: true });
    }
    logger$5.info(`Current working directory: ${process.cwd()}`);
    logger$5.info(`User data path: ${userDataPath$1}`);
    logger$5.info(`Node.js version: ${process.version}`);
    logger$5.info(`Electron version: ${process.versions.electron}`);
    logger$5.info("Loading better-sqlite3 module");
    const Database2 = loadNativeModule("better-sqlite3");
    if (!Database2) {
      const error = new Error("Failed to load better-sqlite3 module");
      connectionState.error = error;
      connectionState.connected = false;
      logger$5.error({ err: error }, "Database initialization failed");
      return fail$1(error);
    }
    logger$5.info("Creating SQLite database instance");
    let sqlite;
    try {
      const dbExists = fs$2.existsSync(dbPath);
      if (!dbExists) {
        logger$5.info(`Database file does not exist at ${dbPath}, creating a new one`);
      }
      sqlite = new Database2(dbPath);
      logger$5.info("SQLite database instance created successfully");
    } catch (dbError) {
      logger$5.error({ err: dbError }, "Failed to create SQLite database instance");
    ...
    try {
      sqlite.pragma("journal_mode = WAL");
      sqlite.pragma("foreign_keys = ON");
      logger$5.info("SQLite pragmas set successfully");
      ensureTablesExist(sqlite);
      const db2 = drizzle(sqlite, { schema });
      logger$5.info("Drizzle instance created successfully");
      try {
        const testQuery = sqlite.prepare("SELECT 1 AS test").get();
        logger$5.info({ testResult: testQuery }, "Database test query executed successfully");
        dbInstance = db2;
...
function getDatabase() {
  ...
  try {
    if (!dbInstance) {
      logger$5.info("Database instance not available, trying to initialize");
      try {
        const Database2 = loadNativeModule("better-sqlite3");
        if (!Database2) {
          throw new Error("Failed to load better-sqlite3 module");
        ...
        ensureTablesExist(sqlite);
        if (!dbExists) {
          logger$5.info("New database detected, schema initialized");
        }
        const testQuery = sqlite.prepare("SELECT 1 AS test").get();
        ...
        connectionState.lastCheckTime = Date.now();
        connectionState.error = null;
        logger$5.info("Database initialized successfully through direct method");
        return dbInstance;
      } catch (initError) {
        logger$5.error({ err: initError }, "Critical error creating database directly");
    ...
    logger$5.error({ err: error }, "Critical error getting database instance");
...
function closeDatabase() {
  if (dbInstance) {
    try {
      ...
      logger$5.info("Database connection closed successfully");
    } catch (error) {
      logger$5.error({ err: error }, "Error closing database connection");
    } finally {
      dbInstance = null;
      connectionState.connected = false;
    }
...
if (isEffect(initialDb)) {
  runPromise(initialDb).then((db2) => {
    logger$5.info("Database initialized successfully on module load");
  }).catch((error) => {
    logger$5.error({ err: error }, "Initial database initialization failed");
  });
}
function ensureTablesExist(sqlite) {
  logger$5.info("Ensuring all required tables exist");
  try {
    ...
    logger$5.info(tableExists ? "Students table exists, ensuring all tables" : "Creating missing tables");
    ...
    logger$5.info("Tables created or verified successfully");
    return true;
  } catch (error) {
    logger$5.error({ err: error }, "Failed to ensure tables exist");
    throw new Error(`Failed to ensure tables exist: ${error instanceof Error ? error.message : String(error)}`);
  }
}
class BaseRepository {
  constructor(table2, logger2, entityName) {
    /**
      ...
      return try_$2({
        try: async () => {
          ...
          try {
            ...
            await database2.insert(this.table).values(insertData);
    ...
    __publicField(this, "getById", (id) => {
      return try_$2({
        try: async () => {
          ...
          try {
            const database2 = getDatabase();
            const entity = await database2.select().from(this.table).where(({ id: entityId }) => entityId.eq(id)).then((rows) => rows[0]);
            if (!entity) {
    ...
    __publicField(this, "update", (id, data) => {
      return try_$2({
        try: async () => {
          ...
          try {
            const database2 = getDatabase();
            const existing = await database2.select().from(this.table).where(({ id: entityId }) => entityId.eq(id)).then((rows) => rows[0]);
            if (!existing) {
            ...
            await database2.update(this.table).set(updatedData).where(({ id: entityId }) => entityId.eq(id));
            const updated = await database2.select().from(this.table).where(({ id: entityId }) => entityId.eq(id)).then((rows) => rows[0]);
      ...
      return try_$2({
        try: async () => {
          ...
          try {
            ...
            await database2.delete(this.table).where(({ id: entityId }) => entityId.eq(id));
    ...
    __publicField(this, "getAll", (filter = {}) => {
      return try_$2({
        try: async () => {
          ...
          try {
            const database2 = getDatabase();
            const entities = await database2.select().from(this.table).where(() => {
              let condition = {};
...
const logsDir$3 = path$2.join(process.cwd(), "logs");
if (!fs$2.existsSync(logsDir$3)) {
  fs$2.mkdirSync(logsDir$3, { recursive: true });
}
const logger$4 = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsDir$3, "student-repository.log")));
class StudentRepository extends BaseRepository {
  constructor() {
    super(students, logger$4, "student");
    /**
    ...
    __publicField(this, "insert", (entity) => {
      return tryPromise({
        try: async () => {
          logger$4.info({ data: entity }, "Creating new student with custom insert method");
          try {
            const id = generateUlid$1();
            const insertData = {
              id,
            ...
            logger$4.info({ insertData }, "Inserting student data");
            const query = db.insert(students).values(insertData);
            logger$4.info({ sql: query.toSQL() }, "Generated SQL for insert");
            await query.run();
            logger$4.info({ id }, "Student created successfully, querying to verify");
            const inserted = await db.select().from(students).where(eq(students.id, id)).then((rows) => rows[0]);
            if (!inserted) {
              logger$4.warn({ id }, "Student was not found after insert - possible DB issue");
              throw new Error("Student was not properly inserted into the database");
            }
            logger$4.info({ student: inserted }, "Student insert verified successfully");
            return inserted;
          } catch (dbError) {
            logger$4.error({ err: dbError }, "Database error during student creation");
            throw new Error(`Failed to create student in database: ${dbError instanceof Error ? dbError.message : "Unknown error"}`);
          }
        },
        catch: (error) => {
          logger$4.error({ err: error }, "Failed to create student");
    ...
    __publicField(this, "kick", (id) => {
      return tryPromise({
        try: async () => {
          logger$4.info({ studentId: id }, "Soft-deleting student");
          const existing = await db.select().from(students).where(eq(students.id, id)).then((rows) => rows[0]);
          if (!existing) {
            logger$4.warn({ studentId: id }, "Student not found for soft deletion");
          ...
          logger$4.info({ studentId: id }, "Student soft-deleted successfully");
          return true;
        },
        catch: (error) => {
          logger$4.error({ err: error, studentId: id }, "Failed to soft-delete student");
    ...
    __publicField(this, "list", (filter = {}) => {
      return tryPromise({
        try: async () => {
          logger$4.info({ filter }, "Getting students with filters");
          ...
          try {
            ...
            logger$4.error({ err: queryError }, "Error executing student query");
            return [];
          }
        },
        catch: (error) => {
          logger$4.error({ err: error }, "Failed to get students with filters");
        ...
        try: async () => {
          try {
            if (ids2 && ids2.length > 0) {
              logger$4.info({ count: ids2.length }, "Bulk exporting specific students");
              ...
              logger$4.info("Bulk exporting all students");
            ...
            logger$4.error({ err: queryError }, "Error executing student bulk export query");
            return [];
          }
        },
        catch: (error) => {
          logger$4.error({ err: error }, "Failed to bulk export students");
    ...
    __publicField(this, "count", (filter = {}) => {
      return tryPromise({
        try: async () => {
          logger$4.info({ filter }, "Counting students with filters");
          ...
          logger$4.info({ count }, "Students counted successfully");
          return count;
        },
        catch: (error) => {
          logger$4.error({ err: error }, "Failed to count students");
     ...
     */
    __publicField(this, "update", (id, data) => {
      logger$4.info({ studentId: id, updateData: data }, "Updating student with enhanced method");
      return tryPromise({
        try: async () => {
          try {
            const database2 = getDatabase();
            const existing = await database2.select().from(students).where(eq(students.id, id)).then((rows) => rows[0]);
            if (!existing) {
              logger$4.warn({ id }, "Student not found for update");
              throw new Error("Student not found");
            }
            const updateData = {
              ...data,
            ...
            logger$4.info({ id, updateData }, "Executing student update");
            await database2.update(students).set(updateData).where(eq(students.id, id));
            const updated = await database2.select().from(students).where(eq(students.id, id)).then((rows) => rows[0]);
            const changedFields = {};
            ...
            logger$4.error({ id, err: dbError, data }, errorMsg);
            throw new Error(errorMsg);
          }
        },
        catch: (error) => {
          logger$4.error({ err: error, studentId: id, updateData: data }, "Failed to update student");
...
const logsDir$2 = path$2.join(process.cwd(), "logs");
if (!fs$2.existsSync(logsDir$2)) {
  fs$2.mkdirSync(logsDir$2, { recursive: true });
}
const logger$3 = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsDir$2, "group-repository.log")));
class GroupRepository extends BaseRepository {
  constructor() {
    super(groups, logger$3, "group");
    /**
    ...
    __publicField(this, "getBySubject", (subjectId) => {
      return try_({
        try: async () => {
          logger$3.info({ subjectId }, "Getting groups by subject ID");
          const result = await db.select().from(groups).where(eq(groups.subjectId, subjectId)).all();
          logger$3.info({ subjectId, count: result.length }, "Groups retrieved successfully");
          return result;
        },
        catch: (error) => {
          logger$3.error({ err: error, subjectId }, "Failed to get groups by subject");
    ...
    __publicField(this, "getEnrolledStudents", (groupId) => {
      return try_({
        try: async () => {
          logger$3.info({ groupId }, "Getting students enrolled in group");
          ...
          logger$3.info({ groupId, count: result.length }, "Enrolled students retrieved successfully");
          return result;
        },
        catch: (error) => {
          logger$3.error({ err: error, groupId }, "Failed to get enrolled students");
    ...
    __publicField(this, "enrollStudent", (studentId, groupId) => {
      return try_({
        try: async () => {
          logger$3.info({ studentId, groupId }, "Enrolling student to group");
          const student = await db.select().from(students).where(and(
            eq(students.id, studentId),
            eq(students.isKicked, false)
          )).then((rows) => rows[0]);
          if (!student) {
            logger$3.warn({ studentId }, "Student not found or is kicked");
          ...
          if (!group) {
            logger$3.warn({ groupId }, "Group not found");
          ...
          if (existingEnrollment) {
            logger$3.warn({ studentId, groupId }, "Student already enrolled in this group");
          ...
          if (enrolledCount >= group.capacity) {
            logger$3.warn({ groupId }, "Group is at maximum capacity");
          ...
          logger$3.info({ studentId, groupId }, "Student enrolled successfully");
          return true;
        },
        catch: (error) => {
          logger$3.error({ err: error, studentId, groupId }, "Failed to enroll student");
    ...
    __publicField(this, "moveStudent", (studentId, fromGroupId, toGroupId) => {
      return try_({
        try: async () => {
          logger$3.info({ studentId, fromGroupId, toGroupId }, "Moving student between groups");
          const currentEnrollment = await db.select().from(enrollments$1).where(and(
            eq(enrollments$1.studentId, studentId),
            eq(enrollments$1.groupId, fromGroupId)
          )).then((rows) => rows[0]);
          if (!currentEnrollment) {
            logger$3.warn({ studentId, fromGroupId }, "Student not enrolled in source group");
          ...
          if (!destGroup) {
            logger$3.warn({ toGroupId }, "Destination group not found");
          ...
          if (existingDestEnrollment) {
            logger$3.warn({ studentId, toGroupId }, "Student already enrolled in destination group");
          ...
          if (enrolledCount >= destGroup.capacity) {
            logger$3.warn({ toGroupId }, "Destination group is at maximum capacity");
          ...
          logger$3.info({ studentId, fromGroupId, toGroupId }, "Student moved successfully");
          return true;
        },
        catch: (error) => {
          logger$3.error({ err: error, studentId, fromGroupId, toGroupId }, "Failed to move student");
  ...
  listAllGroups: () => try_$2({
    try: async () => {
      ...
      try {
        ...
        const allGroups = await database2.query.groups.findMany();
...
const logsDir$1 = path$2.join(process.cwd(), "logs");
if (!fs$2.existsSync(logsDir$1)) {
  fs$2.mkdirSync(logsDir$1, { recursive: true });
}
const logger$2 = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsDir$1, "subject-repository.log")));
class SubjectRepository extends BaseRepository {
  constructor() {
    super(subjects, logger$2, "subject");
    /**
    ...
    __publicField(this, "getSubjectDetails", (id) => {
      return try_$2({
        try: async () => {
          logger$2.info({ subjectId: id }, "Getting subject details");
          const subject = await db.select().from(subjects).where(eq(subjects.id, id)).then((rows) => rows[0]);
          if (!subject) {
            logger$2.warn({ subjectId: id }, "Subject not found");
          ...
          logger$2.error({ err: error, subjectId: id }, "Failed to get subject details");
    ...
    __publicField(this, "getEnrolledStudents", (subjectId) => {
      return try_$2({
        try: async () => {
          logger$2.info({ subjectId }, "Getting students enrolled in subject");
          const result = await db.select({
            id: students.id,
            firstName: students.firstName,
            lastName: students.lastName,
          ...
          logger$2.info({ subjectId, count: result.length }, "Enrolled students retrieved successfully");
          return result;
        },
        catch: (error) => {
          logger$2.error({ err: error, subjectId }, "Failed to get enrolled students");
          return error instanceof Error ? error : new Error("Failed to get enrolled students");
        }
      });
    });
    ...
    __publicField(this, "getSubjectsWithStats", () => {
      return try_$2({
        try: async () => {
          logger$2.info("Getting all subjects with stats");
          try {
            ...
            logger$2.info({ count: (allSubjects == null ? void 0 : allSubjects.length) || 0, rawSubjects: allSubjects }, "Raw subjects from database");
            if (!allSubjects || allSubjects.length === 0) {
              logger$2.info("No subjects found, returning empty array");
              return [];
            }
            const subjectsWithStats = await Promise.all(allSubjects.map(async (subject) => {
              ...
              try {
                let groupCount = 0;
                try {
                  const groupResult = await db.select({ count: db.fn.count() }).from(groups).where(eq(groups.subjectId, subject.id)).all();
                  groupCount = Number(((_a3 = groupResult == null ? void 0 : groupResult[0]) == null ? void 0 : _a3.count) || 0);
                } catch (err2) {
                  logger$2.warn({ subject: subject.id, err: err2 }, "Failed to get group count");
                }
                let studentCount = 0;
                try {
                  const studentResult = await db.select({ count: db.fn.count() }).from(enrollments$1).innerJoin(groups, eq(enrollments$1.groupId, groups.id)).innerJoin(students, eq(enrollments$1.studentId, students.id)).where(
                    and(
                      eq(groups.subjectId, subject.id),
                      eq(students.isKicked, false)
                    )
                  ).all();
                  studentCount = Number(((_b2 = studentResult == null ? void 0 : studentResult[0]) == null ? void 0 : _b2.count) || 0);
                } catch (err2) {
                  logger$2.warn({ subject: subject.id, err: err2 }, "Failed to get student count");
                }
                return {
                  ...subject,
                  groupCount,
                ...
                logger$2.warn({ subject: subject.id, err: subjectError }, "Failed to get stats for subject");
                return {
                  ...subject,
                  groupCount: 0,
                  studentCount: 0
            ...
            logger$2.info({ count: subjectsWithStats.length }, "Subjects with stats retrieved successfully");
            return subjectsWithStats;
          } catch (err2) {
            logger$2.error({ err: err2 }, "Error in getSubjectsWithStats, returning empty array");
            return [];
          }
        },
        catch: (error) => {
          logger$2.error({ err: error }, "Failed to get subjects with stats");
    ...
    __publicField(this, "insert", (data) => {
      return try_$2({
        try: async () => {
          ...
          logger$2.info({ insertData }, "Inserting subject with data");
          ...
          if (!insertedSubject) {
            logger$2.error({ id }, "Subject was not found after insertion");
            throw new Error("Subject was not found after insertion");
          }
          logger$2.info({ id, insertedSubject }, "Subject inserted successfully and verified");
          return {
          ...
          logger$2.error({ err: error, data }, "Failed to insert subject");
        ...
        return pipe(
          try_$2({
            try: async () => {
              ...
              ipcLogger.error({ err: err2 }, "Even direct query failed, returning empty array");
              return [];
            }
          }),
          catchAll((finalError) => {
...
const UtilityService = {
  ...
  backupNow: () => try_({
    try: async () => {
      ...
      const backupsDir = path$2.join(userDataPath2, "backups");
      if (!fs$2.existsSync(backupsDir)) {
        await fs$3.mkdir(backupsDir, { recursive: true });
      }
      const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/:/g, "-").replace(/\..+/, "");
      const backupFilename = `scholario-backup-${timestamp}.db`;
      const backupPath = path$2.join(backupsDir, backupFilename);
      const dbPath2 = path$2.join(process.cwd(), "scholario.db");
      const sourceStream = fs$2.createReadStream(dbPath2);
      const destStream = fs$2.createWriteStream(backupPath);
      ...
      const stats = await fs$3.stat(backupPath);
  ...
  restoreBackup: (filePath) => try_({
    try: async () => {
      ...
      const dbPath2 = path$2.join(process.cwd(), "scholario.db");
      const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/:/g, "-").replace(/\..+/, "");
      const preRestoreBackup = path$2.join(
        electron.app.getPath("userData"),
        "backups",
        `pre-restore-${timestamp}.db`
      );
      const backupsDir = path$2.join(electron.app.getPath("userData"), "backups");
      if (!fs$2.existsSync(backupsDir)) {
        await fs$3.mkdir(backupsDir, { recursive: true });
      }
      if (fs$2.existsSync(dbPath2)) {
        const sourceStream = fs$2.createReadStream(dbPath2);
        const destStream = fs$2.createWriteStream(preRestoreBackup);
        await new Promise((resolve, reject) => {
          sourceStream.pipe(destStream);
          sourceStream.on("error", reject);
          destStream.on("error", reject);
      ...
      const backupStream = fs$2.createReadStream(filePath);
      const dbStream = fs$2.createWriteStream(dbPath2);
      await new Promise((resolve, reject) => {
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("db:updateSchema", async () => {
    try {
      logIpcRequest("db:updateSchema", {});
      const { checkAndUpdateSchema: checkAndUpdateSchema2 } = await Promise.resolve().then(() => updateSchema);
      const result = await checkAndUpdateSchema2();
      console.log("Schema update result:", result);
      logIpcResponse("db:updateSchema", { _tag: "Right", right: result });
      return { _tag: "Right", right: result };
    } catch (error) {
      console.error("Error in schema update handler:", error);
      logIpcResponse("db:updateSchema", { left: toError(error) });
      return { _tag: "Left", left: toError(error) };
    }
  });
  electron.ipcMain.handle("db:generateSchemaReport", async () => {
    try {
      logIpcRequest("db:generateSchemaReport", {});
      const { generateSchemaDiffReport: generateSchemaDiffReport2 } = await Promise.resolve().then(() => updateSchema);
      const result = await generateSchemaDiffReport2();
      console.log("Schema report result:", result);
      logIpcResponse("db:generateSchemaReport", { _tag: "Right", right: result });
      return { _tag: "Right", right: result };
    } catch (error) {
      console.error("Error in schema report handler:", error);
      logIpcResponse("db:generateSchemaReport", { left: toError(error) });
      return { _tag: "Left", left: toError(error) };
    }
  });
  ...
  electron.ipcMain.handle("group:directCreateGroup", async (_, subjectId, data) => {
    try {
      logIpcRequest("group:directCreateGroup", { subjectId, data });
      if (!subjectId) {
        throw new Error("Subject ID is required");
      }
      const { db: db2, generateUlid: generateUlid2 } = await Promise.resolve().then(() => index);
      const { groups: groups2 } = await Promise.resolve().then(() => schema);
      const { eq: eq2 } = await Promise.resolve().then(() => require("./index-Di3SAwUl.js"));
      const id = generateUlid2();
      const now = (/* @__PURE__ */ new Date()).toISOString();
      const groupData = {
        id,
        subjectId,
      ...
      console.log("Direct group creation with data:", groupData);
      await db2.insert(groups2).values(groupData);
      const insertedGroup = await db2.select().from(groups2).where(eq2(groups2.id, id)).then((rows) => rows[0]);
      if (!insertedGroup) {
        throw new Error("Group was not found after insertion");
      }
      console.log("Successfully created group with direct API:", insertedGroup);
      logIpcResponse("group:directCreateGroup", { _tag: "Right", right: insertedGroup });
      return { _tag: "Right", right: insertedGroup };
    } catch (error) {
      console.error("Direct group creation failed:", error);
      logIpcError("group:directCreateGroup", error);
      return { _tag: "Left", left: toError(error) };
    }
  });
  ...
  electron.ipcMain.handle("subject:getDirectSubjects", async () => {
    logIpcRequest("subject:getDirectSubjects", {});
    try {
      const { db: db2 } = await Promise.resolve().then(() => index);
      const { subjects: subjects2 } = await Promise.resolve().then(() => schema);
      const allSubjects = await db2.select().from(subjects2).all();
  ...
  electron.ipcMain.handle("subject:directDeleteSubject", async (_, id) => {
    logIpcRequest("subject:directDeleteSubject", { id });
    try {
      const { db: db2 } = await Promise.resolve().then(() => index);
      const { subjects: subjects2 } = await Promise.resolve().then(() => schema);
      const { eq: eq2 } = await Promise.resolve().then(() => require("./index-Di3SAwUl.js"));
      console.log(`Attempting direct deletion of subject ${id}`);
      const existingSubject = await db2.select().from(subjects2).where(eq2(subjects2.id, id)).all();
      if (!existingSubject || existingSubject.length === 0) {
        console.error(`Subject with ID ${id} not found for direct deletion`);
        return { _tag: "Left", left: { message: `Subject with ID ${id} not found` } };
      }
      await db2.delete(subjects2).where(eq2(subjects2.id, id)).run();
      console.log(`Subject ${id} directly deleted successfully`);
      return { _tag: "Right", right: true };
    } catch (error) {
      console.error("Direct subject deletion failed:", error);
      return { _tag: "Left", left: { message: `Failed to delete subject: ${error instanceof Error ? error.message : String(error)}` } };
    }
  });
  ...
  electron.ipcMain.handle("db:inspectTable", async (_, tableName) => {
    try {
      ...
      const { db: db2 } = await Promise.resolve().then(() => index);
      const result = await db2.all(`PRAGMA table_info("${tableName}")`);
      console.log(`Table schema for ${tableName}:`, result);
      return { _tag: "Right", right: result };
    } catch (error) {
...
function logIpcError(channel, error) {
...
function requireBindings() {
  ...
  (function(module2, exports2) {
    var fs2 = fs$2, path2 = path$2, fileURLToPath = requireFileUriToPath(), join2 = path2.join, dirname = path2.dirname, exists2 = fs2.accessSync && function(path22) {
      try {
...
function requireWrappers() {
  ...
  wrappers$1.loadExtension = function loadExtension(...args2) {
    this[cppdb].loadExtension(...args2);
    return this;
  };
  wrappers$1.defaultSafeIntegers = function defaultSafeIntegers(...args2) {
    this[cppdb].defaultSafeIntegers(...args2);
    return this;
  };
  wrappers$1.unsafeMode = function unsafeMode(...args2) {
...
function requireAggregate() {
  ...
  aggregate = function defineAggregate(name, options) {
    ...
    const step2 = getFunctionOption(options, "step", true);
    ...
    let argCount = -1;
    if (!varargs) {
      argCount = Math.max(getLength(step2), inverse ? getLength(inverse) : 0);
      if (argCount > 0) argCount -= 1;
      if (argCount > 100) throw new RangeError("User-defined functions cannot have more than 100 arguments");
    }
    this[cppdb].aggregate(start, step2, inverse, result, name, argCount, safeIntegers, deterministic, directOnly);
    return this;
  };
...
function requireTable() {
  ...
  function parseTableDefinition(def, verb, moduleName) {
    ...
    const columnDefinitions = [
      ...parameters.map(identifier2).map((str) => `${str} HIDDEN`),
      ...columns.map(identifier2)
    ];
    return [
  ...
  function wrapGenerator(generator, columnMap, moduleName) {
    return function* virtualTable(...args2) {
      const output = args2.map((x) => Buffer.isBuffer(x) ? Buffer.from(x) : x);
  ...
  const identifier2 = (str) => `"${str.replace(/"/g, '""')}"`;
  const defer = (x) => () => x;
  return table;
}
var inspect;
...
const logsDir = path$2.join(process.cwd(), "logs");
if (!fs$2.existsSync(logsDir)) {
  fs$2.mkdirSync(logsDir, { recursive: true });
}
const logger$1 = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsDir, "db-schema-update.log")));
async function checkAndUpdateSchema() {
  try {
    logger$1.info("Checking database schema...");
    const dbPath2 = path$2.join(process.cwd(), "scholario.db");
    if (!fs$2.existsSync(dbPath2)) {
      logger$1.error("Database file not found");
      return { success: false, message: "Database file not found" };
    }
    const sqlite = new Database(dbPath2);
    const groupsTableInfo = sqlite.prepare("PRAGMA table_info(groups)").all();
    logger$1.info({ groupsTableInfo }, "Groups table structure");
    const columnNames = groupsTableInfo.map((col) => col.name);
    const neededColumns = ["id", "subject_id", "name", "capacity", "notes", "created_at", "updated_at"];
    const missingColumns = neededColumns.filter((col) => !columnNames.includes(col));
    if (missingColumns.length > 0) {
      logger$1.warn({ missingColumns }, "Missing columns in groups table");
      for (const col of missingColumns) {
        try {
          logger$1.info(`Adding missing column ${col} to groups table`);
          switch (col) {
            case "capacity":
              sqlite.exec("ALTER TABLE groups ADD COLUMN capacity INTEGER DEFAULT 10");
              break;
            case "notes":
              sqlite.exec("ALTER TABLE groups ADD COLUMN notes TEXT");
              break;
            case "created_at":
              sqlite.exec("ALTER TABLE groups ADD COLUMN created_at TEXT DEFAULT CURRENT_TIMESTAMP");
              break;
            case "updated_at":
              sqlite.exec("ALTER TABLE groups ADD COLUMN updated_at TEXT DEFAULT CURRENT_TIMESTAMP");
              break;
            default:
              logger$1.warn(`Cannot add primary/foreign key column ${col} in SQLite`);
          }
        } catch (colError) {
          logger$1.error({ err: colError }, `Error adding column ${col}`);
        }
      }
    }
    const schemaColumns = ["id", "subject_id", "name", "capacity", "notes", "created_at", "updated_at"];
    const extraColumns = columnNames.filter((col) => !schemaColumns.includes(col));
    if (extraColumns.length > 0) {
      logger$1.warn({ extraColumns }, "Extra columns in groups table");
      logger$1.info("SQLite does not support dropping columns directly. Consider recreating the table if needed.");
    }
    sqlite.close();
    return {
      success: true,
      modified: missingColumns.length > 0,
      message: missingColumns.length > 0 ? `Added ${missingColumns.length} missing columns` : "No schema changes needed"
    };
  } catch (error) {
    logger$1.error({ err: error }, "Error checking/updating database schema");
    return { success: false, message: `Error: ${error instanceof Error ? error.message : String(error)}` };
  }
}
async function generateSchemaDiffReport() {
  try {
    logger$1.info("Generating schema diff report...");
    const dbPath2 = path$2.join(process.cwd(), "scholario.db");
    if (!fs$2.existsSync(dbPath2)) {
      logger$1.error("Database file not found");
      return { success: false, message: "Database file not found" };
    ...
    logger$1.info({ tables: tables.map((t) => t.name) }, "Tables in database");
    const report = {
      tables: {},
      mismatches: []
    };
    for (const tableObj of tables) {
      ...
      if (tableName === "groups") {
        const expectedColumns = ["id", "subject_id", "name", "capacity", "notes", "created_at", "updated_at"];
    ...
    logger$1.error({ err: error }, "Error generating schema diff report");
    return { success: false, message: `Error: ${error instanceof Error ? error.message : String(error)}` };
  }
}
const updateSchema = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  checkAndUpdateSchema,
  generateSchemaDiffReport
}, Symbol.toStringTag, { value: "Module" }));
const isDev = process.env.NODE_ENV === "development";
const getFilePathForEnv = (relativePath) => {
  if (isDev) {
    return path$2.join(process.cwd(), relativePath);
  } else {
    return path$2.join(__dirname, relativePath);
  }
};
const userDataPath = electron.app.getPath("userData");
const logsPath = path$2.join(userDataPath, "logs");
if (!fs$2.existsSync(logsPath)) {
  fs$2.mkdirSync(logsPath, { recursive: true });
}
const logger = pino$1({
  level: "info",
  timestamp: pino$1.stdTimeFunctions.isoTime
}, pino$1.destination(path$2.join(logsPath, "app.log")));
...
function createWindow() {
  ...
  const preloadPath = isDev ? path$2.join(process.cwd(), "preload.js") : path$2.join(__dirname, "../preload.js");
...
electron.app.whenReady().then(async () => {
  try {
    ...
    await ensureCorrectDatabaseSchema();
  } catch (dbError) {
...
async function ensureCorrectDatabaseSchema() {
  try {
    logger.info("Checking database schema consistency...");
    const result = await checkAndUpdateSchema();
    if (result.modified) {
      logger.info("Database schema was updated:", result);
    } else {
      logger.info("Database schema is consistent with code.");
    }
  } catch (error) {
    logger.error({ err: error }, "Error checking database schema");
  }
}
